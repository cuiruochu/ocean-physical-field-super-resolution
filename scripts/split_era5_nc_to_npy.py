...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Split an ERA5 NetCDF file into one .npy file per time step and variable. "
    ...
    parser.add_argument(
        "nc_file",
        type=Path,
        help="Path to the source ERA5 .nc file.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def variable_to_array(dataset: xr.Dataset, variable_name: str, time_index: int) -> np.ndarray:
    values = dataset[variable_name].isel({TIME_DIMENSION: time_index}).to_numpy()
...
def split_nc_to_npy(nc_file: Path, output_dir: Path, overwrite: bool) -> int:
    ...
    with xr.open_dataset(nc_file) as dataset:
        if TIME_DIMENSION not in dataset.dims:
            raise SystemExit(
                f"The ERA5 NetCDF file does not contain a {TIME_DIMENSION!r} dimension."
            )
        if not dataset.data_vars:
            raise SystemExit("The NetCDF file does not contain any data variables.")

        time_values = dataset[TIME_DIMENSION].to_numpy()
        written = 0

        for variable_name in dataset.data_vars:
            if TIME_DIMENSION not in dataset[variable_name].dims:
                raise SystemExit(
                    f"Variable {variable_name!r} does not contain a {TIME_DIMENSION!r} dimension."
                )
